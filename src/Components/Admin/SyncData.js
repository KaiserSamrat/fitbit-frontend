...
const SyncData = () => {
  ...
  const handleSyncData = () => {
    ...
    for (let index = 0; index < users.length; index++) {
      let userID = users?.data?.[index]?._id;

    if(users?.data?.[index]?.fitbit=== true){
      fetch(
        `${API_URL}/users/get-activity-data?activityName=${"elevation"}&startDate=${syncBeforeDate}&endDate=${syncAfterDate}&userid=${userID}`,
        {
          method: "GET",
        ...
        .then((data) => postSyncData(data, userID))
        .catch((err) => {
      ...
      fetch(
        `${API_URL}/users/get-activity-data?activityName=${"steps"}&startDate=${syncBeforeDate}&endDate=${syncAfterDate}&userid=${userID}`,
        {
          method: "GET",
        ...
        fetch(
          `${API_URL}/users/get-activity-data?activityName=${'floors'}&startDate=${syncBeforeDate}&endDate=${syncAfterDate}&userid=${userID}`,
          {
            method: "GET",
          ...
          fetch(
            `${API_URL}/users/get-activity-data?activityName=${'distance'}&startDate=${syncBeforeDate}&endDate=${syncAfterDate}&userid=${userID}`,
            {
              method: "GET",
            ...
            fetch(
              `${API_URL}/users/get-activity-data?activityName=${'calories'}&startDate=${syncBeforeDate}&endDate=${syncAfterDate}&userid=${userID}`,
              {
                method: "GET",
              ...
              fetch(
                `${API_URL}/users/get-activity-data?activityName=${'heart'}&startDate=${syncBeforeDate}&endDate=${syncAfterDate}&userid=${userID}`,
                {
                  method: "GET",
  ...
  const postSyncData = (data, userID) => {
    ...
    if (data?.data?.["activities-elevation"]) {
      ...
      dispatch(SyncByAdminData(obj, history, authtoken, userID));
    }
    ...
    if (data?.data?.["activities-heart"]) {
      ...
      dispatch(syncHeartDataByAdmin(body, history, authtoken, userID));
    }
    setSyncLoading(false);
  };
